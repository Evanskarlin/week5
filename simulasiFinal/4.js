...
function train(pokemon, candy) {
    // code here
    var hasil = "Congratulations, your " + pokemon["name"] + " grew to LV." + (pokemon["level"] + candy) + "!"
    var hasilkurang = ''
    if (pokemon["evolutionLine"] != undefined){
        hasil += " And it evolved into "
        for(var i = 0 ; i<pokemon["evolutionLine"].length ; i++){
            if(pokemon["evolutionLine"][i]["level"] <= pokemon["level"] + candy){
                hasil += pokemon["evolutionLine"][i]["name"] + " and "
            }

        }
        for(var j = 0 ; j<hasil.length-5 ; j++){
          hasilkurang += hasil[j]
        }
        hasilkurang += "!"
    }
